...
String HexEncodingExtensions::EncodeToHex(const StdVectorUint8 &data,
                                          const bool &toUpper) {
  return Botan::hex_encode(data, toUpper);
}

String HexEncodingExtensions::EncodeToHex(const String &data,
                                          const bool &toUpper) {
  return EncodeToHex(ConverterExtensions::StringToStdVectorUint8(data),
                     toUpper);
}
...
StdVectorUint8 Base32EncodingExtensions::DecodeFromBase32(const String &data) {
  USING_NS_INSANE_STR;
  Botan::SecureVector<uint8_t> decoded =
      Botan::base32_decode(StringExtensions::ToUpper(data));
  return StdVectorUint8(decoded.begin(), decoded.end());
}

String Base32EncodingExtensions::EncodeToBase32(const StdVectorUint8 &data,
                                                const bool &removePadding,
                                                const bool &toLower) {
  String encoded = Botan::base32_encode(data);
  encoded =
      removePadding
          ? StringExtensions::Replace(encoded, EQUAL_SIGN_STRING, EMPTY_STRING)
          : encoded;
  return toLower ? StringExtensions::ToLower(encoded) : encoded;
}

String InsaneIO::Insane::Cryptography::Base32EncodingExtensions::EncodeToBase32(
    const String &data, const bool &removePadding, const bool &toLower) {
  return EncodeToBase32(ConverterExtensions::StringToStdVectorUint8(data),
                        removePadding, toLower);
}
...
StdVectorUint8 Base64EncodingExtensions::DecodeFromBase64(const String &data) {
  ...
  USING_NS_INSANE_CORE;
  String base64 = data;
  base64 = StringExtensions::Replace(
      base64, {{URL_ENCODED_PLUS_SIGN_STRING, PLUS_SIGN_STRING},
               {URL_ENCODED_SLASH_STRING, SLASH_STRING},
               {URL_ENCODED_EQUAL_SIGN_STRING, EQUAL_SIGN_STRING},
               {MINUS_SIGN_STRING, PLUS_SIGN_STRING},
               {UNDERSCORE_STRING, SLASH_STRING}});
  base64 = StringExtensions::RemoveBlankSpaces(base64);
  size_t modulo = base64.length() % 4;
  base64 = StringExtensions::PadRight(
      base64, base64.length() + (modulo > 0 ? 4 - modulo : 0),
      EQUAL_SIGN_STRING);
  Botan::secure_vector<uint8_t> result = Botan::base64_decode(base64);
  return StdVectorUint8(result.begin(), result.end());
}

String Base64EncodingExtensions::EncodeToBase64(const StdVectorUint8 &data,
  ...
  if (lineBreaksLength > 0) {
    ret =
        StringExtensions::InsertRepeat(ret, lineBreaksLength, LINE_FEED_STRING);
  }
  ...
  size_t unpad(const uint8_t block[], size_t len) const override {
    ...
    if (pad_value <= len &&
        std::all_of(block + len - pad_value, block + len,
                    [pad_value](uint8_t val) { return val == pad_value; })) {
...
RsaEncryptor::RsaEncryptor(const RsaEncryptor &instance)
    : RsaEncryptor(instance.GetKeyPair(), instance.GetPadding(),
                   instance.GetEncoder()) {}
